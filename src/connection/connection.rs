...
impl Connection {
    pub fn new(
        ...
        let mut stream =
            TcpStream::connect(address).map_err(|e| Error::ConnectionFailed(e.to_string()))?;
    ...
    fn handle_message(&mut self, msg: StellarMessage) -> Result<(), Error> {
        match msg {
            ...
            StellarMessage::Auth(_) => self.complete_handshake()?,
