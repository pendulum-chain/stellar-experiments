...
pub enum InnerMessage {
    HandshakeCompleted,
    MessageFromServer(StellarMessage),
    Problem(String),
}
...
pub struct ConnectionConfig {
    ...
    /// a channel to send messages back to the caller
    stellar_message_writer: Option<mpsc::Sender<InnerMessage>>,
}
...
impl ConnectionConfig {
    ...
    async fn process_raw_message(&mut self, data: &[u8]) -> Result<(), Error> {
        ...
        match msg_type {
            MessageType::Transaction if !self.receive_tx_messages => {
                ...
                // done processing
                // todo: check if this is needed: self.send_sendMore_message()?;
            }
            ...
            _ => {
                ...
                self.process_stellar_message(auth_msg.message).await?;
            }
        }
        Ok(())
    }
    ...
    async fn process_stellar_message(&mut self, msg: StellarMessage) -> Result<(), Error> {
        match msg {
            ...
            StellarMessage::Auth(_) => {
                ...
                sender.send(InnerMessage::HandshakeCompleted).await?;

                self.send_sendMore_message().await?;
            }
            ...
            other => {
                let sender = self
                    ...
                    .as_ref()
                    .ok_or(Error::ChannelNotSet)?;
                sender.send(InnerMessage::MessageFromServer(other)).await?;
            }
        }
        Ok(())
    }
    ...
    async fn send_sendMore_message(&mut self) -> Result<(), Error> {
    ...
    pub fn set_message_writer(&mut self, sender: mpsc::Sender<InnerMessage>) {
        self.stellar_message_writer = Some(sender);
    }
}


/// This seriously needs a new name.
/// Where communication happens with the channels holding the stream.
pub async fn cfg_communication(
    mut cfg: ConnectionConfig,
    mut receiver: mpsc::Receiver<CfgActions>,
) -> Result<(), Error> {
    loop {
        if let Some(actions) = receiver.recv().await {
            match actions {
                CfgActions::SendHello => {
                    cfg.send_hello_message().await?;
                }
                CfgActions::SendMessage(msg) => {
                    cfg.send_stellar_message(msg).await?;
                }
                CfgActions::HandleMessage(xdr) => {
