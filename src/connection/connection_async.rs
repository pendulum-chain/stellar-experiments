...
/*
    pub async fn start(&mut self ) -> Result<(),Error> {
        ...
        loop {
            ...
            self.stream.readable().await.map_err(|e| Error::ReadFailed(e.to_string()))?;

            match self.stream.try_read(&mut readbuf) {
                Ok(read_size) => {
                    let message_len = get_message_length(&readbuf);
                    let message_len = usize::try_from(message_len).unwrap();

                    if message_len > read_size {
    ...
    pub async fn handle_message(&mut self, msg: StellarMessage, tx: mpsc::Sender<StellarMessage>) -> Result<(), Error> {
        match msg {
            ...
            _ => {
                tx.send(msg).await.map_err(|e|Error::Undefined(e.to_string()))?;
            }
        }
        Ok(())
    }
    ...
    pub async fn send_stellar_message2(&mut self, stellar_message_in_bytes:&[u8]) -> Result<usize,Error> {
       self.stream.write(stellar_message_in_bytes).await.map_err(|e| {
            Error::WriteFailed(e.to_string())
        })
    }
    
    pub async fn send_stellar_message(&mut self, msg:StellarMessage) -> Result<(),Error> {
        ...
        let _  = self.stream.write(&xdr_authenticated_msg).await.map_err(|e| {
            Error::WriteFailed(e.to_string())
        })?;
...
//
// pub async fn handle_message(message:StellarMessage, remote_called_us:bool) {
//     match message {
//         StellarMessage::Auth(_) => {
//
//         }
