...
mod test {
    ...
    use crate::{
        ...
        helper::time_now,
        node::NodeInfo,
        ConnConfig, ConnectorActions, StellarNodeMessage,
    };
    ...
    fn create_new_connector_works() {
        ...
        assert_eq!(
            connector_local_node.ledger_version,
            node_info.ledger_version
        );
        assert_eq!(
            connector_local_node.overlay_version,
            node_info.overlay_version
        );
        assert_eq!(
            connector_local_node.overlay_min_version,
            node_info.overlay_min_version
        );
    ...
    fn connector_method_works() {
        ...
        assert_eq!(
            connector.receive_scp_messages(),
            connConfig.recv_scp_messages
        );
    ...
    async fn connector_send_to_user_works() {
        ...
        let message = StellarNodeMessage::Timeout;
    ...
    async fn connector_send_to_node_works() {
        ...
        connector
            .send_to_node(ConnectorActions::SendHello)
            .await
            .unwrap();
