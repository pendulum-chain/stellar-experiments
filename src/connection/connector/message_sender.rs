...
impl Connector {
    ...
    async fn send_stellar_message(&mut self, msg: StellarMessage) -> Result<(), ConnectionError> {
    ...
    pub(super) async fn send_hello_message(&mut self) -> Result<(), ConnectionError> {
        self.send_to_node(ConnectorActions::SendHello)
            .await
            .map_err(ConnectionError::from)
    }

    pub(super) async fn send_auth_message(&mut self) -> Result<(), ConnectionError> {
